defmodule Day23.PartTwo do
  ...
  defp build_graph(curr, prev, target, _map, weight, _set, graph) when curr == target,
    do: Graph.add_edges(graph, [{prev, curr, weight: weight}, {curr, prev, weight: weight}])
  ...
  defp build_graph(curr, prev, target, map, weight, set, graph) do
    ...
    |> case do
      ...
      nexts ->
        graph
        |> Graph.add_edges([{prev, curr, weight: weight}, {curr, prev, weight: weight}])
        |> then(fn graph ->
